...
export default class App extends Component {
  ...
  render() {
    ...
    if (this.state.loc) {
      ...
      locText = `Your Position is: ${latitude}:${longitude}`
    } else if(this.state.errMes) {
      locText = this.state.errMes
    }
    return (
      <View style={styles.container}>
        <Text style={styles.welcome}>Press the Button below to get Your Location!</Text>
        <Button title="Get Location" onPress={this.getLocationHandler} />
        <Text style={styles.welcome}>{locText}</Text>
      </View>
    );
  }
}
